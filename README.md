...
- 最近 365 期向前回測的第 `t` 期只使用 `[:t]`，並同時呈現重作模型、舊版模型及每期兩組隨機選號的最佳命中數。
- 報告公開每組各因子相對分數及「至少中 2 碼」比較；分數只用於排序，不是假裝成中獎機率。
...
GitHub Actions 可在 Actions 頁手動觸發，亦於台灣開獎日週一至週六晚上 21:15、21:30、21:45、22:00 自動嘗試更新。流程先測試、抓取資料並產生報告；內容有變更才提交，因此重複執行不會重複寫入。單次工作最長 15 分鐘，避免外部服務或程式異常時長時間占用。若台灣彩券封鎖 GitHub runner，工作會明確失敗而不會靜默改用非官方來源。
