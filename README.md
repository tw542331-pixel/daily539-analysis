...
- 最近 365 期滾動回測的第 `t` 期僅將 `[:t]` 傳給策略，並比較每期兩組隨機選號的最佳命中數分布。
- 選號先依近百期統計排序，再由 16 碼候選池開始分層搜尋；條件不足時自動擴大，避免每天重算全部 575,757 種組合。
...
GitHub Actions 可在 Actions 頁手動觸發，亦於台灣開獎日週一至週六開獎後（UTC 13:30，即台灣 21:30）執行。流程先測試、更新資料和報告，再提交變更；單次工作最長 15 分鐘，避免外部服務或程式異常時長時間占用。若台灣彩券封鎖 GitHub runner，工作會明確失敗而不會靜默改用非官方來源。
